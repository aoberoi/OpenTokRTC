...
for (var reservation in reservations) {
  (function(r) {
    r.opentok.createSession(function(err, session) {
      ...
      r.token = r.opentok.generateToken(r.sessionId, { role: 'moderator' });
    });
...
module.exports = function(app) {
  app.get(/^\/tnwdemo(.json)?$/, function(req, res, next) {
    ...
    req.token = reservations.tnwdemo.token;
    req.apiKey = reservations.tnwdemo.apiKey;
    console.log('attached reservation data to request', req.sessionId, req.token, req.apiKey);
    next();
  });
};
